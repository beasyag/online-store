...
const selectRoom = (roomId: number) => {
  if (activeRoomId.value === roomId && pendingSellerId.value === null) return;
...
const openRoom = async (roomId: number) => {
  ...
  if (room) {
    try {
      const roomMessages = await api.get<any>(`/chat/rooms/${roomId}/messages/`);
      if (activeRoomId.value === roomId) {
        messages.value = Array.isArray(roomMessages) ? roomMessages : (roomMessages?.results || []);
      }
    } catch (error) {
      ...
      <main class="flex flex-1 flex-col bg-slate-50/50" :class="{ 'hidden sm:flex': !activeRoomId }">
        ...
        <template v-else>
          ...
          <div ref="messagesContainer" class="flex-1 overflow-y-auto p-4 space-y-4">
            <div v-if="roomLoading" class="text-center text-xs text-slate-400 my-4">Загрузка сообщений...</div>
