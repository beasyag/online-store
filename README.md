...
If the PostgreSQL container logs `Database directory appears to contain a database; Skipping initialization` followed by authentication errors such as `Role "postgres" does not exist`, the named Docker volume was initialized with older credentials. For disposable local data, reset it and start from a fresh database:
