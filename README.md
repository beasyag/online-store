...
- `+2`, если цена близкая (до 15% разницы — 2 балла, до 30% — 1 балл);
- `+2`, если пересекаются теги.

### Сигналы пользователя (с учётом времени)

Каждый сигнал умножается на коэффициент временного затухания — недавние действия весят больше. Период полураспада: 14 дней.
...
- `+0..2` бонус за популярность (35% просмотры, 45% покупки, 20% рейтинг).

### Коллаборативная фильтрация

- Анализ совместных покупок: «Пользователи, купившие X, также покупали Y».
- На странице товара работает блок «С этим товаром покупают».
- Вес коллаборативного сигнала в персональных рекомендациях: `+4` (убывает по позиции в рейтинге).

### Трендовые товары

- Оценка скорости продаж за последние 7 дней относительно среднего за всё время.
- Товары с растущей динамикой получают бонус `+3` в персональных рекомендациях.
- Отдельный эндпоинт `GET /api/recommendations/trending/` для секции «Сейчас в тренде».

### Диверсификация результатов

- Ограничение по категориям: не более `limit / 4` товаров из одной категории.
- Ограничение по продавцам: не более `limit / 4` товаров от одного продавца.
- Если лимит не заполняется — добавляются оставшиеся товары без ограничений.

### Правила

- Купленные товары исключаются из персональной выдачи;
- Дубликаты по offer_group удаляются;
...
- Рекомендации rule-based с коллаборативной фильтрацией, без ML-модели.
